...
void GameView::initView() {
    ...
    auto handBg = Sprite::create("HandArea.png");
    if (handBg) {
    ...
    auto clubJModel = _model->getDeskCard_ClubJ();
    if (clubJModel) {
        CardView* clubJView = CardView::create(clubJModel);
        if (clubJView) {
            _layer->addChild(clubJView, 2);
            _cardViewMap[clubJModel] = clubJView;
        }
    }
