...
bool CardView::init(CardModel* model) {
    ...
    if (cardSprite) {
        ...
        if (model) {
            ...
            if (suitSprite) {
                suitSprite->setPosition(Vec2(200, 320));
                cardSprite->addChild(suitSprite, 1);
            }

            // 左上角数字
            std::string rankFileA = "card/small_" + colorStr + "_" + rankStr + ".png";
            auto rankSpriteA = Sprite::create(rankFileA);
            if (rankSpriteA) {
                rankSpriteA->setPosition(Vec2(60, 320));
                cardSprite->addChild(rankSpriteA, 1);
            }
            ...
            if (rankSprite) {
                rankSprite->setPosition(Vec2(125, 150));
